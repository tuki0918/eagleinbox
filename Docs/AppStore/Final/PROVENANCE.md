...
- Text and UI are composed deterministically. `04-pro-upgrade.png` uses the approved DEBUG-only screenshot presentation mode, which displays `Unlock Pro` without a localized price for a public, locale-neutral product-page image.
- Only `05-action-button.png` includes a black, white-text `PRO` badge, composed deterministically by the renderer.
- `04-pro-upgrade.png` uses a direct 1260 x 2736 px Simulator capture from the DEBUG-only screenshot presentation mode; no UI pixels or copy were synthesized or retouched.
