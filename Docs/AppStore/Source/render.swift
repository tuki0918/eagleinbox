...
private func render(
    ...
    let hasEyebrow = slide.eyebrow != nil
    let isSingleLineHeadline = !slide.headline.contains("\n")
    let headlineTop: CGFloat = hasEyebrow ? 215 : (isSingleLineHeadline ? 215 : 135)
    let subtitleTop: CGFloat = hasEyebrow ? 390 : (isSingleLineHeadline ? 390 : 445)

    if let eyebrow = slide.eyebrow {
        drawText(
            eyebrow,
            rect: topRect(55, 135, 1150, 70),
            font: subtitleFont,
    ...
    drawText(
        ...
        font: headlineFont,
        color: .white,
        lineHeight: headlineFont.pointSize * 146 / 126,
        tracking: -1.8,
        alignment: .center
    )
...
private let englishSlides: [Slide] = [
    Slide(
...
private let japaneseSlides: [Slide] = [
    Slide(
        headline: "まとめてEagleへ",
        subtitle: "選んだメディアを一度に送信",
        eyebrow: "写真もファイルも",
        badge: nil,
        source: "Docs/Screenshots/ja/upload-queue-photo-url.png",
        secondarySource: nil,
        output: "Docs/AppStore/Final/ja/01-send-items.png",
        background: vividBackground,
        accent: NSColor(hex: 0x5EEAD4)
    ),
    Slide(
        headline: "「共有」から\nすばやく起動",
        subtitle: "写真やファイル、Webページを送信",
        eyebrow: nil,
        badge: nil,
        source: "Docs/Screenshots/ja/share-menu.png",
        secondarySource: nil,
        output: "Docs/AppStore/Final/ja/02-share-sheet.png",
        background: vividBackground,
        accent: NSColor(hex: 0x7DD3FC)
    ),
    Slide(
        headline: "ちょっと整理",
        subtitle: "Eagleのタグやフォルダを送信前に設定",
        eyebrow: nil,
        badge: nil,
        source: "Docs/Screenshots/ja/folders-selected-recent-all.png",
        secondarySource: "Docs/Screenshots/ja/tags-selected.png",
        output: "Docs/AppStore/Final/ja/03-organize-tags.png",
        background: vividBackground,
        accent: NSColor(hex: 0xFDBA74)
    ),
    Slide(
        headline: "Proでもっと自由に",
        subtitle: "接続先は無制限、すべてのショートカットが使える",
        eyebrow: nil,
        badge: nil,
        source: "Docs/Screenshots/ja/pro-upgrade.png",
        secondarySource: nil,
        output: "Docs/AppStore/Final/ja/04-pro-upgrade.png",
        background: vividBackground,
        accent: NSColor(hex: 0xFBBF24)
    ),
    Slide(
        headline: "ボタンひとつで\nEagleへ",
        subtitle: "アクションボタンからショートカットを実行",
        eyebrow: nil,
        badge: "PRO",
        source: "Docs/Screenshots/ja/action-button-shortcut.png",
        secondarySource: nil,
        output: "Docs/AppStore/Final/ja/05-action-button.png",
        background: vividBackground,
        accent: NSColor(hex: 0xC4B5FD)
    )
]

private enum RenderLocale: String {
    case en
    case ja
}

private var arguments = Array(CommandLine.arguments.dropFirst())
private var locale = RenderLocale.en
if let localeFlagIndex = arguments.firstIndex(of: "--locale") {
    guard arguments.indices.contains(localeFlagIndex + 1),
          let parsedLocale = RenderLocale(rawValue: arguments[localeFlagIndex + 1]) else {
        throw NSError(
            domain: "EagleInboxStoreRender",
            code: 9,
            userInfo: [NSLocalizedDescriptionKey: "Use --locale en or --locale ja"]
        )
    }
    locale = parsedLocale
    arguments.removeSubrange(localeFlagIndex...(localeFlagIndex + 1))
}

private let slides = locale == .ja ? japaneseSlides : englishSlides
private let headlineFont = locale == .ja ? japaneseHeadlineFont : englishHeadlineFont
private let subtitleFont = locale == .ja ? japaneseSubtitleFont : englishSubtitleFont
private let requestedOutput = arguments.first
